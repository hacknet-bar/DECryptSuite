        ...
        */
        ...
        static void Help(string LanguageCode = "",string command = "")
        {
            ...
            string[] zh_CN_Help =
            {
                ...
                "",
                "默认加密文件，如果要解密请添加-d或--decrypt",
                "获取帮助 --help <command>",
                "大多数命令都可以使用短参数及其较长的参数执行，例如-c/--credits。（WeiAI：我把长参数都隐藏了#滑稽，要看长参数去把系统语言调成English#滑稽）",
                "-f (--file) / -dir (--folder), -he(--header) 都是必要的参数（-he只有加密时是必要的）",
                "",
                "命令列表:",
                "  -c\t\t\t关于",
                "  -d\t\t\t解密指定的Dec文件",
                "  -dcrack\t\t破解指定的Dec文件（无需密码）",
                "  -f\t\t\t目标文件",
                "  -dir\t\t\t目标目录.",
                "  Source\t\t被指定的文件/目录.",
                "  -a\t\t\t表示一个 ASCII 文本文件。（以ASCII解密/加密）",
                "  -b\t\t\t表示一个二进制文件。（以byte解密/加密）",
                "  -o\t\t\t设定输出目标.",
                "  Destination\t\t输出文件",
                "  -p\t\t\t设定一个密码去加密/解密文件",
                ...
                "  --contact\t\t显示我的联系方式.",
                "  --headeronly\t\tOnly decrypts the header of a file.（需要配合-d使用）"
            };
